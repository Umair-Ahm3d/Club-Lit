  ...
  return (
    <div>
      ...
      <RequestContainer>
        ...
        <h2 style={{ marginTop: '2rem', color:'black' }} >Your Requests</h2>
        {requests.length > 0 ? (
          requests.map(request => (
            <StatusCard key={request._id} status={request.status}>
              <p style={{color:'black'}}><strong>{request.bookTitle}</strong></p>
              <p style={{color:'black'}}>Status: <strong>{request.status}</strong></p>
              {request.status === "Rejected" && request.reason && (
                <p style={{color:'black'}} ><strong>Reason:</strong> {request.reason}</p>
              )}
            </StatusCard>
          ))
        ) : (
          <p style={{color:'black'}}>No book requests yet.</p>
        )}
      </RequestContainer>
      <Footer />
    </div>
